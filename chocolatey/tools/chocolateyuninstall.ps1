...
$key = Get-UninstallRegistryKey -SoftwareName 'ReelGeek*'
...
elseif ($key.Count -eq 0) {
  Write-Warning 'ReelGeek is not installed, or was installed outside of Chocolatey. Nothing to do.'
}
else {
  Write-Warning "$($key.Count) matches found for ReelGeek. Remove it manually rather than guessing:"
