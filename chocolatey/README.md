# Chocolatey package for ReelGeek

The package source for `reelgeek` on the Chocolatey community repository lives here so that
`packageSourceUrl` in the nuspec points at something real.

## What the package does

It downloads `ReelGeekSetup.exe` from the GitHub release for the matching tag and verifies it against a
...
2. Take the SHA-256 for `ReelGeekSetup.exe` from the release's own `SHA256SUMS.txt`.
...
5. Install locally from the built nupkg and check the shim works:
   `choco install reelgeek -s . -y`
6. `choco push reelgeek.<version>.nupkg --source https://push.chocolatey.org/`
